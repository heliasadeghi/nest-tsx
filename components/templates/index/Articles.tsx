// Assuming this is located in a file named Articles.tsx inside a components folder
import React, { useState } from "react";
import Card from "@/components/modules/Card/Card";

// Define the type for the article item
type ArticleItem = {
  id: number;
  title: string;
  svg: JSX.Element;
...
type ArticlesState = ArticleItem[];
...
const Articles: React.FC = () => {
  ...
  return (
    <div className="container px-4 sm:px-10  flex justify-center items-center flex-col mt-16">
      <div className="title mx-auto  flex justify-center items-center flex-col text-center">
        <h1 className="text-[24px] font-extrabold sm:text-[45px] sm:font-bold text-[var(--bg-gray-03)] capitalize">
          Advanced Statistics
        </h1>
        <p className="text-[var(--bg-gray-02)] mt-3 sm:mt-0 text-[16px] sm:text-[18px] font-medium">
          Track how your links are performing across the web with <br /> our
          advanced statistics dashboard
        </p>
      </div>
      <div className="mx-auto relative w-full  sm:h-[300px] flex sm:flex-row flex-col gap-5 mt-20 h-[800px]">
        {data.map((item) => (
          <Card key={item.title} {...item} />
        ))}
        <div className="line absolute w-2 h-full sm:w-full sm:h-1 bg-[var(--bg-cyan)] sm:top-[calc(50%_-_2px)] left-[calc(48%)] sm:left-0"></div>
      </div>
    </div>
  );
};
